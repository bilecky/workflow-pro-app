...
interface SingleProjectProps {
 ...
 const SingleProject: React.FC<SingleProjectProps> = ({ sortedProjects }) => {
	...
	const handleCardClick = (projectId: string) => {
	  if (!currentUser) {
		 setShowTooltips((prevTooltips) => ({ ...prevTooltips, [projectId]: true }));
 
		 setTimeout(() => {
			setShowTooltips((prevTooltips) => ({ ...prevTooltips, [projectId]: false }));
		 }, 2000);
	  }
	};
	return (
		  ...
		  {currentUser ? (
				...
				{sortedProjects.map((project) => (
				  <Link to={`/projects/${project.id}`} key={project.id}>
					 <div className='bg-zinc-300  shadow-custom  mb-4 hover:scale-95 transition-all text-center h-[400px] '>
						...
						<p className='text-gray-500 mb-2 line-clamp-4 px-2 '>{project.description}</p>
			 ...
			 <div className='min-h-[800px] grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 lg:w-4/5 m-auto '>
				{sortedProjects.map((project) => (
				  ...
				  >
					 {showTooltips[project.id] && !currentUser && (
